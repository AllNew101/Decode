...
public class Autonomous_RedNEAR extends OpMode {
    ...
    private final Pose scorePose = new Pose(-20, -20, Math.toRadians(44)); // Scoring Pose of our robot. It is facing the goal at a 135 degree angle.
    private final Pose P_keep1 = new Pose(-40, -30, Math.toRadians(88));
    private final Pose keep1 = new Pose(-40, -8, Math.toRadians(88));
    private final Pose pre = new Pose(-46, -10, Math.toRadians(89));
    private final Pose open = new Pose(-46, -5, Math.toRadians(88));
    private final Pose scorePose2 = new Pose(-25, -28, Math.toRadians(50));
    private final Pose P_keep2 = new Pose(-60.5, -25, Math.toRadians(87));
    private final Pose keep2 = new Pose(-60.5, -5, Math.toRadians(87));
    private final Pose scorePose3 = new Pose(-50, -34, Math.toRadians(46));
    private final Pose P_keep3 = new Pose(-88, -30, Math.toRadians(88));
    private final Pose keep3 = new Pose(-88, -8, Math.toRadians(88));
    private final Pose scorePose4 = new Pose(-50, -36, Math.toRadians(48));
    private final Pose Final = new Pose(-60, -20, Math.toRadians(46));
    private final double ar = 0.72;
    private final double power_adj = 0.52;
    ...
    public void autonomousPathUpdate() throws InterruptedException {
        switch (pathState) {
            case 0:
                ...
                shooter1.setPower(0.68);
                shooter2.setPower(0.68);
            ...
            case 101:
                if (!follower.isBusy()){
                    ...
                    for (int i = 0;i < 3;i++){
                        switch (i) {
                            case 0:
                                shooter1.setPower(power_adj);
                                shooter2.setPower(power_adj);
                                break;
                            case 1:
                                shooter1.setPower(power_adj);
                                shooter2.setPower(power_adj);
            ...
            case 1:
                if (!follower.isBusy()){
                    ...
                    follower.setMaxPower(1);
                    follower.followPath(Pre_keep1,true);
                    shooter1.setPower(power_adj);
                    shooter2.setPower(power_adj);
                    setPathState(2);
                    break;
            ...
            case 2:
                if (!follower.isBusy()){
                    ...
                    follower.followPath(keep_1,true);
                    setPathState(-2);
                    break;
                }
            case -2:
...
//                    if ((follower.getPose().getX() < (keep1.getX() + 1) && (follower.getPose().getY()) < (keep1.getY()) + 1)){
            ...
            case 4:
                if (!follower.isBusy()){
                    front_motor.setPower(1);
                    follower.setMaxPower(1);
                    follower.followPath(Pre_keep2,true);
                    shooter1.setPower(power_adj);
                    shooter2.setPower(power_adj);
                    setPathState(5);
                    break;
...
//                    if ((follower.getPose().getX() < (P_keep2.getX() + 1) && (follower.getPose().getY()) < (P_keep2.getY()) + 1)){
                    follower.setMaxPower(0.6);
                    follower.followPath(keep_2,true);
                    setPathState(6);
                    break;
                }
            case 6:
...
//                    if ((follower.getPose().getX() < (keep2.getX() + 1) && (follower.getPose().getY()) < (keep2.getY()) + 1)){
            ...
            case 7:
                if (!follower.isBusy()){
                    front_motor.setPower(1);
                    follower.setMaxPower(1);
                    follower.followPath(Pre_keep3,true);
                    center_motor.setPower(0.8);
                    shooter1.setPower(power_adj);
                    shooter2.setPower(power_adj);
                    setPathState(8);
...
//                    if ((follower.getPose().getX() < (P_keep3.getX() + 1) && (follower.getPose().getY()) < (P_keep3.getY()) + 1)){
                    follower.setMaxPower(0.6);
                    follower.followPath(keep_3,true);
                    setPathState(9);
                    break;
                }
            case 9:
...
//                    if ((follower.getPose().getX() < (keep3.getX() + 1) && (follower.getPose().getY()) < (keep3.getY()) + 1)){
            ...
            case 10:
                if (!follower.isBusy()){
                    follower.setMaxPower(1);
                    follower.followPath(Finale,true);
